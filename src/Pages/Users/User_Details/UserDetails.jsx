...
export default function UserDetails() {
    ...
    const userDetails = UsersData.find(user => user.id === Number(id));

    // ====== display-file-page ====== //

    const [displayFilePage, setDisplayFilePage] = useState(false);
    const [fileUrl, setFileUrl] = useState(null);

    const openFilePage = (url) => {

        setDisplayFilePage(true);
        setFileUrl(url)
    ...
    return <React.Fragment>
        ...
        <AnimatePresence>
            ...
            {displayFilePage && <PdfViewer fileUrl={fileUrl} display={setDisplayFilePage} />}
        ...
        <div className={userDetailsCSS.container}>
            ...
            <div className={userDetailsCSS.registration_files}>
                ...
                {userDetails.files.map(
                    (files, idx) => <button key={idx} className={userDetailsCSS.file_cont} onClick={() => openFilePage(files.url)}>
                        ...
                        <p>{t(files.name)} | {idx + 1}</p>
                    </button>
                )}

            </div>

            <div className={userDetailsCSS.actions}>

                <button className={userDetailsCSS.cancel_btn}>{t('cancelWord')}</button>

                <button className={userDetailsCSS.accept_btn}>{t('acceptWord')}</button>
